...
FIFOServer::FIFOServer(std::shared_ptr<Afina::Storage> storage)
    : _storage(storage), _reading_fifo(), _is_running(false), _is_stopping(false), _executor(storage), _has_out(false) {
}

FIFOServer::~FIFOServer() { Stop(); }
...
void FIFOServer::Stop() {
    if (!_is_running.load() || _is_stopping.load()) {
        return;
    }
...
void FIFOServer::_ThreadFunction() {
    while (_is_running.load()) {
        ...
        if (result == FIFO::FIFO_READING_STATE::ERROR) {
            if (!_is_stopping.load()) {
                throw POSIXException("Unable to read from pipe!");
            } else {
                break;
            }
        }

        if (result != FIFO::FIFO_READING_STATE::TIMEOUT) {
            _executor.AppendAndTryExecute(new_data);
        }
        ...
        if (_executor.HasOutputData()) {
            ...
            if (result.state == FIFO::FIFO_WRITING_STATE::ERROR) {
                throw POSIXException("Unable write to pipe!");
            }
            if (result.state == FIFO::FIFO_WRITING_STATE::OK) {
                _executor.RemoveFromOutput(result.count_written);
            }
        }
    }
    ...
    if (_executor.HasOutputData()) {
        _writing_fifo.Write(_executor.GetOutputAsIovec(), _executor.GetQueueSize());
    }
}
