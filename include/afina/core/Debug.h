...
#define ASSERT(X)                                                                                                      \
    if (!(X)) {                                                                                                        \
        CURRENT_PROCESS_DEBUG("EXCEPTION: " << #X);                                                                    \
        throw std::runtime_error(#X);                                                                                  \
    }
#define ASSERT_INT_FUNCTION_FOR_ZERO(X, ...)                                                                           \
    int __result = X;                                                                                                  \
    if (__result != 0) {                                                                                               \
        __VA_ARGS__;                                                                                                   \
        CURRENT_PROCESS_DEBUG(#X << " failed with code " << __result);                                                 \
        ASSERT(__result == 0);                                                                                         \
    }
...
#define VALIDATE_CONDITION(X)                                                                                          \
    if (!(X)) {                                                                                                        \
        throw Afina::POSIXException((#X));                                                                             \
    }
